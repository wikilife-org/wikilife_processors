...
from wikilife_processors.processors.stats.was.factors.base_factors_model import \
    WeeklySumAvgFactor, Factor
from wikilife_processors.tests.base_test import BaseTest
from wikilife_utils.date_utils import DateUtils


TEST_USER_ID = "TEST_WSAV"
TEST_NODE_ID = 1

class WeeklySumAvgFactorTests(BaseTest):
    ...
    def _create_factor(self, weekly_avg_day_delta):
        ...
        weekly_avg_day_delta = 21
        factor = WeeklySumAvgFactor(id="test", name="Test", daos=self._daos, value_type=Factor.BUCKET, values=values, day_delta=weekly_avg_day_delta)
        return factor
    ...
    def _get_test_logs_21_days(self):
        ...
        """
        test_logs = [
            {'day': 0,  'value': 5,  'expected': ( (5)/1.0 )/1.0},
            {'day': 1,  'value': 8,  'expected': (5+8)/2.0},
            {'day': 2,  'value': 12, 'expected': (5+8+12)/3.0},
            {'day': 6,  'value': 11, 'expected': (5+8+12+11)/4.0},
            {'day': 6,  'value': 2,  'expected': (5+8+12+11+2)/4.0},
            {'day': 7,  'value': 7,  'expected': (9.5 + (7)/1.0 )/2.0},
            {'day': 7,  'value': 4,  'expected': (9.5 + (7+4)/1.0 )/2.0},
            {'day': 10, 'value': 16, 'expected': (9.5 + (7+4+16)/2.0 )/2.0},
            {'day': 10, 'value': 2,  'expected': (9.5 + (7+4+16+2)/2.0 )/2.0},
            {'day': 10, 'value': 3,  'expected': (9.5 + (7+4+16+2+3)/2.0 )/2.0},
            {'day': 11, 'value': 12, 'expected': (9.5 + (7+4+16+2+3+12)/3.0 )/2.0},
            {'day': 12, 'value': 18, 'expected': (9.5 + (7+4+16+2+3+12+18)/4.0 )/2.0},
            {'day': 14, 'value': 9,  'expected': (9.5 + 15.5 + (9)/1.0 )/3.0},
            {'day': 15, 'value': 18, 'expected': (9.5 + 15.5 + (9+18)/2.0 )/3.0},
            {'day': 15, 'value': 5,  'expected': (9.5 + 15.5 + (9+18+5)/2.0 )/3.0},
            {'day': 16, 'value': 14, 'expected': (9.5 + 15.5 + (9+18+5+14)/3.0 )/3.0},
            {'day': 17, 'value': 3,  'expected': (9.5 + 15.5 + (9+18+5+14+3)/4.0 )/3.0},
            {'day': 20, 'value': 18, 'expected': (9.5 + 15.5 + (9+18+5+14+3+18)/5.0 )/3.0},
        ]
    ...
    def _get_test_logs_22_days(self):
        ...
        test_logs = [
            ...
            {'day': 6,  'value': 2,  'expected': (8+12+11+2)/3.0},
            {'day': 7,  'value': 7,  'expected': (8+12+11+2+7)/4.0},
            
            {'day': 10, 'value': 3,  'expected': ( 10 + 3 )/2.0},
            {'day': 11, 'value': 12, 'expected': ( 10 + (3+12)/2.0 )/2.0},
            {'day': 14, 'value': 18, 'expected': ( 10 + (3+12+18)/3.0 )/2.0},

            {'day': 15, 'value': 18, 'expected': ( 10 + 11 + 18 )/3.0},
            {'day': 15, 'value': 5,  'expected': ( 10 + 11 + (18+5) )/3.0},
            {'day': 17, 'value': 3,  'expected': ( 10 + 11 + (18+5+3)/2.0 )/3.0},
            {'day': 20, 'value': 18, 'expected': ( 10 + 11 + (18+5+3+18)/3.0 )/3.0},
            {'day': 21, 'value': 15, 'expected': ( 10 + 11 + (18+5+3+18+15)/4.0 )/3.0},
            {'day': 21, 'value': 5,  'expected': ( 10 + 11 + (18+5+3+18+15+5)/4.0 )/3.0},
            {'day': 21, 'value': 10, 'expected': ( 10 + 11 + (18+5+3+18+15+5+10)/4.0 )/3.0}
        ]
    ...
    def _get_test_logs_22_days_missing_second_week(self):
        ...
        test_logs = [
            ...
            {'day': 15, 'value': 18, 'expected': ( 10 + 18 )/2.0},
            {'day': 15, 'value': 5,  'expected': ( 10 + (18+5) )/2.0},
            {'day': 17, 'value': 3,  'expected': ( 10 + (18+5+3)/2.0 )/2.0},
            {'day': 20, 'value': 18, 'expected': ( 10 + (18+5+3+18)/3.0 )/2.0},
            {'day': 21, 'value': 15, 'expected': ( 10 + (18+5+3+18+15)/4.0 )/2.0},
            {'day': 21, 'value': 5,  'expected': ( 10 + (18+5+3+18+15+5)/4.0 )/2.0},
            {'day': 21, 'value': 10, 'expected': ( 10 + (18+5+3+18+15+5+10)/4.0 )/2.0}
        ]
