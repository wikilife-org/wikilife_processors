...
from wikilife_processors.processors.stats.was.factors.base_factors_model import \
    Factor, DailySumAvgFactor
from wikilife_processors.tests.base_test import BaseTest
from wikilife_utils.date_utils import DateUtils


TEST_USER_ID = "TEST_DSAV"
TEST_NODE_ID = 1

class DailySumAvgFactorTests(BaseTest):
    ...
    def _create_factor(self, daily_avg_day_delta):
        ...
        factor = DailySumAvgFactor(id="test", name="Test", daos=self._daos, value_type=Factor.BUCKET, values=values, day_delta=daily_avg_day_delta)
        return factor
    ...
    def _get_test_logs_f_v_f(self):
        ...
        test_logs = [
            ...
            {'day': 3,  'value': 8,  'expected': (5+8+12+7+8)/4.0},
            {'day': 4,  'value': 9,  'expected': (5+8+12+7+8+9)/5.0},
            {'day': 4,  'value': 1,  'expected': (5+8+12+7+8+9+1)/5.0},
            {'day': 4,  'value': 2,  'expected': (5+8+12+7+8+9+1+2)/5.0},
            {'day': 6,  'value': 3,  'expected': (5+8+12+7+8+9+1+2+3)/6.0},
            {'day': 6,  'value': 2,  'expected': (5+8+12+7+8+9+1+2+3+2)/6.0}
        ]
    ...
    def _get_test_logs_v_v_f(self):
        ...
        test_logs = [
            ...
            {'day': 3,  'value': 8,  'expected': (12+7+8)/2.0},
            {'day': 4,  'value': 9,  'expected': (12+7+8+9)/3.0},
            {'day': 4,  'value': 1,  'expected': (12+7+8+9+1)/3.0},
            {'day': 4,  'value': 2,  'expected': (12+7+8+9+1+2)/3.0},
            {'day': 6,  'value': 3,  'expected': (12+7+8+9+1+2+3)/4.0},
            {'day': 6,  'value': 2,  'expected': (12+7+8+9+1+2+3+2)/4.0}
        ]
