# coding=utf-8
from wikilife_processors.processors.stats.calories_processor import \
    CaloriesProcessor
...
class CaloriesProcessorTests(BaseTest):
    ...
    def setUp(self):
        self.db = self.get_conn().test_personal_stats_calories
        self.get_conn().drop_database("test_personal_stats_calories")
        ...
        self.calories_prc = CaloriesProcessor(logger, mgr_locator)

    def tearDown(self):
        self.get_conn().drop_database("test_personal_stats_calories")

    def test_accept_valid_log(self):
        log = self._get_calories_valid_log()

        assert self.calories_prc.accept(log) == True

    def test_accept_invalid_log(self):
        log = {}

        assert self.calories_prc.accept(log) == False
    ...
    def test_process(self):
        valid_log = self._get_calories_valid_log()
        ...
        try:
            self.calories_prc.process(valid_log)
            stats = self.calories_prc._get_stats(self.stats_mgr, user_id, week_id)
            
            print stats["sum_calories"]
            
            assert stats["sum_calories"] ==  [1.0, 2.0, 3.0, 4.0, 1508.0, 6.0, 7.0]
    ...
    def test_process_delete(self):
        ...
        try:
            self.calories_prc.process_delete(valid_log)
            stats = self.calories_prc._get_stats(self.stats_mgr, user_id, week_id)
            
            print stats["sum_calories"]
            
            assert stats["sum_calories"] ==  [1.0, 2.0, 3.0, 4.0, 1000.0, 6.0, 7.0]
    ...
    def _get_calories_valid_log(self):
    ...
    def _create_test_initial_data(self, user_id, week_id, start_value):
        ...
        stats["sum_calories"] = [1.0, 2.0, 3.0, 4.0, 1254.0, 6.0, 7.0]
        stats["count_calories"] = [3, 3, 3, 2, 2, 2, 1]
        ...
        nutrients = {"tag_name": "ENERC_KCAL", "value": 254}
        cholesterol = {"tag_name": "CHOLE", "value": 19}
        node["fields"]["nutrients"] = []
        node["fields"]["nutrients"].append(nutrients)
