...
from wikilife_processors.processors.stats.pee_processor import PeeProcessor


class PeeProcessorTests(BaseTest):
    ...
    def setUp(self):
        self.db = self.get_conn().test_personal_stats_pee
        self.get_conn().drop_database("test_personal_stats_pee")
        ...
        self.pee_prc = PeeProcessor(logger, mgr_locator)

    def tearDown(self):
        self.get_conn().drop_database("test_personal_stats_pee")

    def test_accept_valid_log(self):
        log = self._get_pee_valid_log()

        assert self.pee_prc.accept(log) == True

    def test_accept_invalid_log(self):
        log = {}

        assert self.pee_prc.accept(log) == False
    ...
    def test_process(self):
        valid_log = self._get_pee_valid_log()
        ...
        #try:
        self.pee_prc.process(valid_log)
        stats = self.pee_prc._get_stats(self.stats_mgr, user_id, week_id)
        
        print stats["pee"]
        
        assert stats["pee"] == [[("Dark Brown")],[("Dark Brown")],[("Dark Brown")],[("Dark Brown")],[("Dark Brown"), ("Dark Brown")],[("Dark Brown")],[("Dark Brown")]]
    ...
    def test_process_delete(self):
        ...
        #try:
        self.pee_prc.process_delete(valid_log)
        stats = self.pee_prc._get_stats(self.stats_mgr, user_id, week_id)
        
        print stats["pee"]
        
        assert stats["pee"] == [[("Dark Brown")],[("Dark Brown")],[("Dark Brown")],[("Dark Brown")],[],[("Dark Brown")],[("Dark Brown")]]
    ...
    def _get_pee_valid_log(self):
        return {u'pk': 36, u'model': u'LogEntry', u'fields': {u'status': 1, u'execute_time_utc': datetime.datetime(2011, 7, 15, 15, 19, 31), u'server_id': 36, u'user_id': u'DG3NQF', u'client_id': 15, u'update_date': u'2011-07-15', u'text': u'pee', u'create_time_utc': datetime.datetime(2011, 7, 15, 18, 10, 58, 911000), u'execute_date': u'2011-07-15', u'update_time': u'2011-07-15 18:10:58 +0000', u'execute_time': u'2011-07-15 15:19:31 +0000', u'source': u'client.iphone', u'original_entry': 0, u'create_time': u'2011-07-15 18:10:58 +0000', u'root_slug': u'pee', u'create_date': u'2011-07-15', u'nodes': [{u'node_namespace': u'wikilife.physiological.pee', u'node_id': 1090, u'value': 'Dark Brown', u'title': u'pee'}, {u'node_namespace': u'wikilife.pee.intercourse.satisfaction.value-node', u'node_id': 1090, u'value': 10, u'title': u'pee'}, {u'node_namespace': u'wikilife.pee.general.birth-control.methods', u'node_id': 1090, u'value': 'Dark Brown', u'title': u'pee'} ], u'user_log': 1}}
    ...
    def _create_test_initial_data(self, user_id, week_id, start_value):
        ...
        stats["count_pee"] = [2, 2, 2, 2, 2, 2, 2]
        stats["pee"] = [[("Dark Brown")],[("Dark Brown")],[("Dark Brown")],[("Dark Brown")],[("Dark Brown")],[("Dark Brown")],[("Dark Brown")]]       
